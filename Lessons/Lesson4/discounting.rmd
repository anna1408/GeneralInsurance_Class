...
summary(dt_PaidCase)
```
2 typy dat: case su rezervy a paid je to co sa vyplatilo

ay kedy sa stala nehoda

dy je odkial je ten casovy rad...o kolko rokov neskor pochadza to cislo ..v ktorom roku sa stala nehoda a o kolko rokov neskor mam to cislo
...
```
spodne riadky...smpl a vwtd...akym cislom mam prenasobit jeden bod aby som sa dostal na koniec
...pouziva sa skor vwtd...treba pozriet help...asi sa skor lisia iba v tom ze jedno z nich ignoruje NA

_!!!!!!!!!!!!!!!!!!!!Odtialto domaca Uloha!!!!!!!!!!!!!!!!!!_

_Now repeat for all types of buiness and sizes of claims. Compare the findings..._

_Homework (~30 min) [4b / 2b]_
Finish exercises 2 and 3 in Discounting.
...
tu som to zopakovala len pre dataset type Paid (nakolko sme predtym brali PAID a v zadani ste nenapisali ci to chcete aj pre CASE alebo aj spolu)

```{r}
P_H_S <- dt_PaidCase %>% filter(Business == "House" & ClaimSize == "Small" & dataset_type == "PAID")

P_H_L <- dt_PaidCase %>% filter(Business=="House" & ClaimSize=="Large" & dataset_type=="PAID") 

P_3P_S <- dt_PaidCase %>% filter(Business=="3rd Party" & ClaimSize=="Small" & dataset_type=="PAID")

P_3P_L <- dt_PaidCase %>% filter(Business=="3rd Party" & ClaimSize=="Large" & dataset_type=="PAID")
```

```{r}
P_H_S_triangle <- P_H_S %>% as.triangle(P_H_S, origin = "ay", dev = "dy", value = "SumOfamount")
P_H_S_triangle

P_H_L_triangle <- P_H_L %>% as.triangle(P_H_L, origin = "ay", dev = "dy", value = "SumOfamount")
P_H_L_triangle

P_3P_S_triangle <- P_3P_S %>% as.triangle(P_3P_S, origin = "ay", dev = "dy", value = "SumOfamount")
P_3P_S_triangle

P_3P_L_triangle <- P_3P_L %>% as.triangle(P_3P_L, origin = "ay", dev = "dy", value = "SumOfamount")
P_3P_L_triangle
```

```{r}
plot(P_H_S_triangle)
plot(predict(chainladder(P_H_S_triangle)))
ata(P_H_S_triangle)
```
na grafe pekne mozeme vidiet ako nam to pekne skonverguje to znamena ze naklady sa zastabilizuju....su tu teda kratke chvosty

```{r}
plot(P_H_L_triangle)
plot(predict(chainladder(P_H_L_triangle)))
ata(P_H_L_triangle)
```
tu to uz take pekne ako v predoslom pripade nie je...zjavne tam vyskocili naklady, ale nasledne to tiez pekne skonvergovalo takze aj tu mame kratke chvosty


```{r}
plot(P_3P_S_triangle)
plot(predict(chainladder(P_3P_S_triangle)))
ata(P_3P_S_triangle)
```
tuto krasne vidime dlhe chvosty, to znamena ze nam to neskonvergovalo a naklady rastu spolu s nasledujucimi rokmi

```{r}
plot(P_3P_L_triangle)
plot(predict(chainladder(P_3P_L_triangle)))
ata(P_3P_L_triangle)

```
v predoslych pripadoch boli hodnoty ktore sme prediskovali spolu...tu vidime ako nam "jedna krivka" uletela od ostatnych...je to asi sposobene vysokymi hodnotami v prvom stlpci v tabulke co asi znamena ze v jednom roku naklady prudko vyskocili
tu neviem naisto povedat ci ide o short alebo long tail...tie "krivky" ktore su pokope su short ale ta jedna "uletena" by bola long

co sa tyka porovnania medzi small and large claims tak ten tu mame viditelny pri domacnostiach ...large nam skonverguje rychlejsie ako small...pri autach sa to chova uplne odlisne

ked porovname domacnosti a auta tak mozeme skonstatovat ze su to dve uplne odlisne kategorie...pri domacnostiach naklady nam konvergovali...asi preto ze ked nastane nejaka skoda na budove tak po jej  oprave uz nebude treba dalsie opravy a naklady s tym spojene zatial co pri autonehodach, treba opravit auto a mozu tam nastat aj nejake skryte poruchy ktore sa ukazu neskor, clovek ktory je zraneny tak moze mat nasledky z toho velmi dlho dokym sa da dokopy a tak...


ak by bolo treba spravit CASE tak tu je len neokomentovany kod

```{r}
C_H_S <- dt_PaidCase %>% filter(Business == "House" & ClaimSize == "Small" & dataset_type == "CASE")

C_H_L <- dt_PaidCase %>% filter(Business=="House" & ClaimSize=="Large" & dataset_type=="CASE") 

C_3P_S <- dt_PaidCase %>% filter(Business=="3rd Party" & ClaimSize=="Small" & dataset_type=="CASE")

C_3P_L <- dt_PaidCase %>% filter(Business=="3rd Party" & ClaimSize=="Large" & dataset_type=="CASE")


C_H_S_triangle <- C_H_S %>% as.triangle(C_H_S, origin = "ay", dev = "dy", value = "SumOfamount")
C_H_S_triangle

C_H_L_triangle <- C_H_L %>% as.triangle(C_H_L, origin = "ay", dev = "dy", value = "SumOfamount")
C_H_L_triangle

C_3P_S_triangle <- C_3P_S %>% as.triangle(C_3P_S, origin = "ay", dev = "dy", value = "SumOfamount")
C_3P_S_triangle

C_3P_L_triangle <- C_3P_L %>% as.triangle(C_3P_L, origin = "ay", dev = "dy", value = "SumOfamount")
C_3P_L_triangle

plot(C_H_S_triangle)
plot(predict(chainladder(C_H_S_triangle)))
ata(C_H_S_triangle)

plot(C_H_L_triangle)
plot(predict(chainladder(C_H_L_triangle)))
ata(C_H_L_triangle)

plot(C_3P_S_triangle)
plot(predict(chainladder(C_3P_S_triangle)))
ata(C_3P_S_triangle)

plot(C_3P_L_triangle)
plot(predict(chainladder(C_3P_L_triangle)))
ata(C_3P_L_triangle)
```
...
netusim ci aj toto bolo treba...z vasho zadania na stranke to jasne nebolo!!!


```{r}
PaC_H_S <- dt_PaidCase %>% filter(Business == "House" & ClaimSize == "Small")

PaC_H_L <- dt_PaidCase %>% filter(Business=="House" & ClaimSize=="Large") 

PaC_3P_S <- dt_PaidCase %>% filter(Business=="3rd Party" & ClaimSize=="Small")

PaC_3P_L <- dt_PaidCase %>% filter(Business=="3rd Party" & ClaimSize=="Large")


PaC_H_S_triangle <- PaC_H_S %>% as.triangle(PaC_H_S, origin = "ay", dev = "dy", value = "SumOfamount")
PaC_H_S_triangle

PaC_H_L_triangle <- PaC_H_L %>% as.triangle(PaC_H_L, origin = "ay", dev = "dy", value = "SumOfamount")
PaC_H_L_triangle

PaC_3P_S_triangle <- PaC_3P_S %>% as.triangle(PaC_3P_S, origin = "ay", dev = "dy", value = "SumOfamount")
PaC_3P_S_triangle

PaC_3P_L_triangle <- PaC_3P_L %>% as.triangle(C_3P_L, origin = "ay", dev = "dy", value = "SumOfamount")
PaC_3P_L_triangle
```
```{r}
plot(PaC_H_S_triangle)
plot(predict(chainladder(PaC_H_S_triangle)))
ata(PaC_H_S_triangle)
```
tu naklady s postupujucim casom pomalicky klesaju


```{r}
plot(PaC_H_L_triangle)
plot(predict(chainladder(PaC_H_L_triangle)))
ata(PaC_H_L_triangle)
```
vyzera to velmi stabilne 

```{r}
plot(PaC_3P_S_triangle)
plot(predict(chainladder(PaC_3P_S_triangle)))
ata(PaC_3P_S_triangle)
```
vyzera to nestabilne, moze to byt sposobene vyssou volatilitou

```{r}
plot(PaC_3P_L_triangle)
plot(predict(chainladder(PaC_3P_L_triangle)))
ata(PaC_3P_L_triangle)
...
short tail ...rychlo to skonverguje k tomu finalnemu
data ktore nekonverguju hned su long tail
